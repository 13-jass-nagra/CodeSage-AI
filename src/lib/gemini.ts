...
// A utility function to handle retries with exponential backoff
const withRetryAndBackoff = async <T>(
  fn: () => Promise<T>,
  retries = 5,
  delay = 1000
): Promise<T> => {
  try {
    ...
    if (error.response?.status === 429 && retries > 0) {
      ...
      return withRetryAndBackoff(fn, retries - 1, delay * 2);
    }
    throw error;
  }
};
...
export async function summariseCode(doc: Document) {
  ...
  try {
    ...
    const response = await withRetryAndBackoff(async () => {
...
export async function generateEmbedding(summary: string) {
  const result = await withRetryAndBackoff(async () => {
    return embeddingModel.embedContent(summary);
  });
...
//   const response = await model.generateContent([
//     // presumably: diff or instructions go here
//    `You are an expert programmer, and you are trying to summarize a git diff.
// Reminders about the git diff format:
// For every file, there are a few metadata lines, like (for example):
// \`\`\`
